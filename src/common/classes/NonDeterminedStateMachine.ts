...
export default class NonDeterminedStateMachine {
  ...
  private _availableTransitions: Map<string, Record<string, boolOrCallbackToBool>> = new Map();
  private _stateHistory: string[] = [];
  private _historyLimit = 10;

  private _addToHistory(state: string): void {
    this._stateHistory.push(state);
    if (this._stateHistory.length > this._historyLimit) this._stateHistory.shift();
  }

  constructor(states: Set<string>, transitions: Map<string, Record<string, boolOrCallbackToBool>>) {
